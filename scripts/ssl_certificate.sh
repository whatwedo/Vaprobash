...
if [[ $CA_FILENAME != "false" ]]; then
	...
	sudo openssl req -new -config $DOMAIN.conf -keyout $DOMAIN.key -out $DOMAIN.csr
	sudo openssl x509 -req -in $DOMAIN.csr -CA $CA_FILENAME.pem -CAkey $CA_FILENAME.key -CAcreateserial -out $DOMAIN.crt -days 365 -sha256 -extfile $DOMAIN.ext -passin file:passphrase.txt
fi

# Copy cert (have to be DOMAIN.EXT) files and replace xip once
